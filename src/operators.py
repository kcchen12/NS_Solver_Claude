...
def _u_ghost_top(u: np.ndarray, bc: BoundaryConfig) -> np.ndarray:
...
def _v_ghost_right(v: np.ndarray, bc: BoundaryConfig) -> np.ndarray:
...
def divergence(u: np.ndarray, v: np.ndarray,
    ...
    """
    dx = grid.dx_cells[:, np.newaxis]
    dy = grid.dy_cells[np.newaxis, :]
...
def laplacian_u(u: np.ndarray, grid: CartesianGrid,
    ...
    u_xp = u[2:, :]   # u[i+1, j]
    u_0 = u[1:-1, :]  # u[i,   j]
    hx_m = grid.dx_cells[:-1][:, np.newaxis]
    hx_p = grid.dx_cells[1:][:, np.newaxis]
    d2u_dx2 = 2.0 * (
        u_xm / (hx_m * (hx_m + hx_p))
        - u_0 / (hx_m * hx_p)
        + u_xp / (hx_p * (hx_m + hx_p))
    ...
    u_ghost_top = _u_ghost_top(u[1:-1, :], bc)
    ...
    hy_m = (grid.yc_with_ghost[1:-1] - grid.yc_with_ghost[:-2])[np.newaxis, :]
    hy_p = (grid.yc_with_ghost[2:] - grid.yc_with_ghost[1:-1])[np.newaxis, :]
    d2u_dy2 = 2.0 * (
        u_ym / (hy_m * (hy_m + hy_p))
        - u_0 / (hy_m * hy_p)
        + u_yp / (hy_p * (hy_m + hy_p))
...
def laplacian_v(v: np.ndarray, grid: CartesianGrid,
    ...
    v_yp = v[:, 2:]    # v[i, j+1]
    v_0 = v[:, 1:-1]  # v[i, j]
    hy_m = grid.dy_cells[:-1][np.newaxis, :]
    hy_p = grid.dy_cells[1:][np.newaxis, :]
    d2v_dy2 = 2.0 * (
        v_ym / (hy_m * (hy_m + hy_p))
        - v_0 / (hy_m * hy_p)
        + v_yp / (hy_p * (hy_m + hy_p))
    ...
    v_ghost_rgt = _v_ghost_right(v[:, 1:-1], bc)
    ...
    hx_m = (grid.xc_with_ghost[1:-1] - grid.xc_with_ghost[:-2])[:, np.newaxis]
    hx_p = (grid.xc_with_ghost[2:] - grid.xc_with_ghost[1:-1])[:, np.newaxis]
    d2v_dx2 = 2.0 * (
        v_xm / (hx_m * (hx_m + hx_p))
        - v_0 / (hx_m * hx_p)
        + v_xp / (hx_p * (hx_m + hx_p))
...
def convection_u(u: np.ndarray, v: np.ndarray,
    ...
    dx_u = grid.dxc[:, np.newaxis]
    duu_dx = (u_e**2 - u_w**2) / dx_u
    ...
    u_ghost_top = _u_ghost_top(u_int, bc)
    ...
    dy_u = grid.dy_cells[np.newaxis, :]
    dvu_dy = (v_n * u_n - v_s * u_s) / dy_u
...
def convection_v(u: np.ndarray, v: np.ndarray,
    ...
    v_ghost_rgt = _v_ghost_right(v_int, bc)
    ...
    dx_v = grid.dx_cells[:, np.newaxis]
    duv_dx = (u_e * v_e - u_w * v_w) / dx_v

    # --- d(vv)/dy ---
    v_n = 0.5 * (v_int + v[:, 2:])    # (v[j] + v[j+1]) / 2
    v_s = 0.5 * (v[:, :-2] + v_int)   # (v[j-1] + v[j]) / 2

    dy_v = grid.dyc[np.newaxis, :]
    dvv_dy = (v_n**2 - v_s**2) / dy_v
